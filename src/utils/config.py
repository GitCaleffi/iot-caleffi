...
def validate_connection_string(connection_string):
    ...
    # Accept both device-level and service-level connection strings
    device_required_parts = ['HostName', 'DeviceId', 'SharedAccessKey']
    service_required_parts = ['HostName', 'SharedAccessKeyName', 'SharedAccessKey']
    
    try:
        parts = dict(part.split('=', 1) for part in connection_string.split(';'))
        
        # Check if it's a device-level connection string
        device_valid = all(key in parts for key in device_required_parts)
        # Check if it's a service-level connection string
        service_valid = all(key in parts for key in service_required_parts)
        
        if device_valid or service_valid:
            return True
        else:
            print(f"Invalid connection string format. Must contain either:")
            print(f"Device format: {device_required_parts}")
            print(f"Service format: {service_required_parts}")
